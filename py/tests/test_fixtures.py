...
asserted exactly for these:
  ...
  - Transport scenarios (scenario.json fixtures: AGUI501/505/506/507/508)
    are skipped entirely — they need the transport layer (PM5), which
...
@pytest.mark.skip(reason="needs the transport layer (PM5), not yet ported")
@pytest.mark.parametrize("dir_path", sorted(d for d in _INVALID_DIRS if d.name in _TRANSPORT_DIRS), ids=lambda p: p.name)
def test_transport_fixture_fires_its_namesake_rule(dir_path):
    pass
